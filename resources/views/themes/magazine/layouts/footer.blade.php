<footer class="relative w-full h-max isolate  @if (!$themeSettings->footer_color || mode()) bg-theme-three @endif"
    @if (!mode() && $themeSettings->footer_color) style="background-color: {{ $themeSettings->footer_color }};" @endif>
    <div class="container py-6">
        @include ('themes.magazine.components.common.footer-heading')

        <section
            class="grid md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-4 xl:gap-0 border-y border-y-sky-800 py-8 lg:divide-x lg:divide-sky-800">
            <!--  {/* Address Section */} -->
                ...
                <form id="subscribe-form" class="relative my-6 max-w-xl mx-auto">
                    ...
                    <div class="relative">
                        <input name="subscriber_email" id="subscriber_email"
                            class="text-neutral-800 py-3 pl-3 pr-28 rounded-md w-full duration-300 ease-in placeholder:text-sm border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500"
                            type="email" placeholder="Email" required />

                        <button type="submit" id="subscribe-btn"
                            class="absolute top-1/2 -translate-y-1/2 right-0 rounded-r-md bg-blue-500 text-white hover:bg-blue-600 px-4 h-full border border-blue-500 flex items-center justify-center gap-1 transition-all duration-300 ease-in">
                            {{ localize('subscribe') }}
                        </button>
                    </div>
                </form>
            ...
            <ul class="flex items-center gap-4 capitalize mt-4 md:mt-0">
                @if ($footerPages->isNotEmpty())
                    @foreach ($footerPages as $footerPage)
                        ...
                        <li>
                            <a class="nav_link_theme_5" href="{{ $link }}">{{ $footerPage->menu_level }}</a>
                        </li>
                    @endforeach
                @endif
            </ul>
