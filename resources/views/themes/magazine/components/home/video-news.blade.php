<!-- Video News Swiper -->
<div class="border-b border-blue-900 pb-4 overflow-x-auto overflow-y-hidden">
    <div class="divide-x divide-blue-900 rtl:divide-x-reverse flex">
        @foreach ($videoNews as $videoNewsItem)
            <!-- Card-1 -->
            <div class="px-2 2xl:px-3 xl:first:ps-0 xl:last:pr-0 py-4 lg:py-0 min-w-[320px]">
                ...
                <div class="p-2 space-y-2">
                    <a href="{{ __url($videoNewsItem->encode_title) }}"
                        class="text-white hover:text-cyan-500 dark:hover:text-cyan-500 font-bold text-lg leading-6 line-clamp-2 transition_3">
                        {{ $videoNewsItem ->title }}
                    </a>

                    <div class="text-neutral-200 capitalize flex items-center gap-1 text-xs">
                        ...
                        <span>{{ news_publish_date_format($videoNewsItem->publish_date) }}</span></span>
                    </div>
                </div>
            </div>
        @endforeach
