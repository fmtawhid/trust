...
@foreach ($imageLibraries as $imageLibrary)
    @php
        ...
        $thumbRelativePath = 'uploads/photo-library/' . basename($imageLibrary->thumb_image);
        $publicFilePath = public_path($thumbRelativePath);

        if (!file_exists($publicFilePath)) {
            $thumbRelativePath = $imageLibrary->thumb_image;
            $publicFilePath = public_path($thumbRelativePath);
        }

        $version = file_exists($publicFilePath) ? filemtime($publicFilePath) : time();
        $image_path = asset($thumbRelativePath);
    @endphp
